...
export default function transformImportAtrule(rule, opts) {
	...
	if (opts.transform.includes('@import')) {
		...
		const parser =
			options.parser || (options.syntax && options.syntax.parse) || null

		if (
			(opts.importFilter instanceof Function &&
				opts.importFilter(id, media)) ||
			...
			return importPromise.then(
				...
				({ file, contents }) =>
					processor
						.process(contents, { from: file, parser: parser })
						.then(({ root }) => {
...
const trimWrappingURL = (string) =>
	trimWrappingQuotes(string.replace(/^url\(([\W\w]*)\)$/, '$1'))
